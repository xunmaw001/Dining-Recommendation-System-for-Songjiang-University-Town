...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusscantingmeishiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusscantingmeishiCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusscantingmeishi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusscantingmeishi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discusscantingmeishi', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusscantingmeishi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusscantingmeishi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discusscantingmeishi', this.detail);
				this.$utils.msg('审核成功');
