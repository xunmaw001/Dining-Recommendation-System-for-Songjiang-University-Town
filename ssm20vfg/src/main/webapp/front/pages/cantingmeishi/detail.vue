      	...
      	<swiper-item :style='{"padding":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,0)","backgroundColor":"rgba(255,255,255,1)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"300rpx"}' v-for="(swiper,index) in swiperList" :key="index">
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop" :style='{"padding":"0px","boxShadow":"rgb(0 0 0 / 0%) 0px 0px 12rpx","margin":"0px","borderColor":"rgb(204, 204, 204)","backgroundColor":"rgb(255, 255, 255)","borderRadius":"0px","borderWidth":"2rpx 0 0 0","width":"100%","borderStyle":"solid","height":"100rpx"}'>
				...
				<view style="text-align: right;">
					<button v-if="userid&&isAuth('cantingmeishi','评价')" @tap="onAcrossTap('pingjiaxinxi','','[1]','你已评价')" class="cu-btn bg-brown round shadow-blur" style="margin-right: 10upx;min-width: 100upx;" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[0+6],borderColor:btnColor[0+6]}">评价</button>
					<button v-if="!userid&&isAuthFront('cantingmeishi','评价')" @tap="onAcrossTap('pingjiaxinxi','','[1]','你已评价')" class="cu-btn bg-brown round shadow-blur" style="margin-right: 10upx;min-width: 100upx;" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[0+6],borderColor:btnColor[0+6]}">评价</button>
                </view>
			</view>
		</view>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getStoreup();
			this.getThumbsup();
			let cleanType = uni.getStorageSync('discusscantingmeishiCleanType')
			if(cleanType){
				uni.removeStorageSync('discusscantingmeishiCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','cantingmeishi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','cantingmeishi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('cantingmeishi', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discusscantingmeishi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusscantingmeishi/add-or-update?refid=${this.id}`)
			},
			// 获取赞踩
			async getThumbsup() {
				let params = {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('cantingmeishi', _this.detail);
								_this.$utils.msg('取消成功');
			...
			async onFinishTap() {
				...
				await this.$api.update('cantingmeishi', this.detail);
				this.$utils.msg('审核成功');
